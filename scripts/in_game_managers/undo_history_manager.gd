...
func _ready() -> void: 
	managers.atom_slots_manager.atom_added.connect(_on_atom_added)
...
#		return 
#	await managers.atom_slots_manager.atom_added
...
func create_turn_data() -> TurnData: 
	return TurnData.new(
		managers.atom_slots_manager.atom_slots_saved_data.duplicate(),  
		managers.atom_players_manager.atom_players_in_play.duplicate(), 
		managers.atom_player_turns_manager.turn_index, 
...
#	history.pop_back()
	...
	if !is_instance_valid(turn_data): 
		...
		for atom_player in managers.atom_players_manager.atom_players_in_play: 
			atom_player.reset()
	if managers.atom_players_manager.eliminated_atom_players.size() > 0: 
		var atom_players_in_play: Array[AtomPlayer] = turn_data.atom_players_in_play
		for atom_player in atom_players_in_play: 
			if managers.atom_players_manager.atom_players_in_play.has(atom_player): 
				print("You cannot undo once a player has been eliminated. ")
				return
	managers.atom_slots_manager.apply_undo_changes(turn_data)
	managers.atom_players_manager.apply_undo_changes(turn_data)
	managers.atom_player_turns_manager.apply_undo_changes(turn_data)
	for atom_player in managers.atom_players_manager.atom_players_in_play: 
		var current_total_atom_count: int = managers.atom_players_manager.get_current_total_atoms_count(atom_player)

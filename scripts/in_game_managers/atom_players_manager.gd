...
#	managers.atom_slots_manager.atom_slot_exploded.connect(_on_atom_slot_exploded)
...
#	await get_tree().create_timer(3, false).timeout
	if managers.atom_player_turns_manager.is_chain_reacting(): 
		await managers.atom_slots_manager.atom_slot_exploded
	var atom_count: int = get_current_total_atoms_count(atom_player)
	if GameManager.debug: 
		print("managers.atom_players_manager: %s: Atom Count: %s" % [atom_player.group_name, atom_count])
	if atom_count <= 0: 
		if (atom_player in atom_players_in_play) == false: 
			return
		managers.atom_players_manager.elimnate_team(atom_player)
...
func start_game(_player_amount: int) -> void: 
	...
	managers.atom_player_turns_manager.start_game()
...
func elimnate_team(atom_player: AtomPlayer) -> void: 
	...
	if GameManager.debug: 
		print("managers.atom_players_manager: %s has been eliminated!" % atom_player.group_name)
		print("managers.atom_players_manager: Current teams remaining: %s" % atom_players_remaining)

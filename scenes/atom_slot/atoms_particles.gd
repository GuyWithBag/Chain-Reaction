...
#	var particle_id: String = particle_name.to_snake_case() 
	...
	#var managers = world.managers
	var current_team_color: Color = PlayersManager.current_player_in_turn.team_color
	var team_color: Color = current_team_color
	if owner.previous_player: 
		if owner.previous_player.team_color != current_team_color: 
			team_color = owner.previous_player.team_color
